...
# Constants
ALERT_THRESHOLD = 40  # Minimum number of frames with alerts
FRAME_BUFFER_SIZE = 60  # The number of frames to store per camera
...
# Dictionary to store timestamps for each frame at each stage
frame_times = {}

# Helper function to print latency
def print_latency(stage, frame_number, start_time):
    latency = (time.time() - start_time) * 1000  # Convert to milliseconds
    print(f"{stage} latency for frame {frame_number}: {latency:.2f} ms")

def add_timed_probe(element, pad_name, stage_name):
    pad = element.get_static_pad(pad_name)
    if not pad:
        sys.stderr.write(f"Unable to get {pad_name} pad for {stage_name}\n")
        return
    ...
    def probe_callback(pad, info):
        ...
        # Initialize dictionary for the frame if not present
        if frame_number not in frame_times:
            frame_times[frame_number] = {}

        # Log the time for the current stage
        if stage_name not in frame_times[frame_number]:
            frame_times[frame_number][stage_name] = current_time
            if stage_name != "Stream-muxer":  # Skip latency for first stage
                previous_stage = list(frame_times[frame_number].keys())[-2]
                print_latency(f"{previous_stage} -> {stage_name}", frame_number,
                              frame_times[frame_number][previous_stage])

        return Gst.PadProbeReturn.OK

    # Add the probe callback
...
def log_alert(camera_index, average_confidence):
    # Map the index to the camera name using the dictionary
    camera_name = camera_name_map.get(camera_index, f"Camera_{camera_index}")  # Default name if index not found

    # Get the current date and time
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    date, time = timestamp.split(" ")

    # Write to the CSV file with camera name, date, time, and confidence in separate columns
    with open(CSV_FILENAME, mode='a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([camera_name, date, time, f"{average_confidence:.2f}"])  # Add confidence to the row
    print(f"Logged alert for {camera_name} at {date} {time} with avg confidence {average_confidence:.2f}")
...
def pgie_src_pad_buffer_probe(pad, info, u_data):
    ...
    while l_frame is not None:
        ...
        for bbox, confidence, obj_meta in objects:
            ...
            obj_meta.rect_params.border_width = 1  # Adjust this value for the desired thickness
...
def main(args, requested_pgie=None, config=None, disable_probe=False):
    ...
    # Place add_timed_probe calls here before starting the pipeline
    add_timed_probe(streammux, "src", "Stream-muxer")
    add_timed_probe(pgie, "src", "Primary-inference")
    add_timed_probe(tiler, "sink", "Tiler")
    add_timed_probe(nvosd, "src", "On-Screen Display (OSD)")
    add_timed_probe(sink, "sink", "Sink")
