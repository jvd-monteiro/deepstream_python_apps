...
# Function to log the camera and timestamp when the alert condition is met
def log_alert(camera_index):
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with open(CSV_FILENAME, mode='a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([camera_index, timestamp])
    print(f"Logged alert for camera {camera_index} at {timestamp}")
...
def pgie_src_pad_buffer_probe(pad, info, u_data):
    ...
    while l_frame is not None:
        ...
        obj_counter = {
            PGIE_CLASS_ID_SCAPCOL: 0,
            PGIE_CLASS_ID_SCAP: 0,
            PGIE_CLASS_ID_SCOL: 0,
        }
        ...
        while l_obj is not None:
            ...
            # Set the bounding box color based on the class
            if obj_meta.class_id in [PGIE_CLASS_ID_SCAPCOL, PGIE_CLASS_ID_SCAP, PGIE_CLASS_ID_SCOL]:
                # Set to red for specific classes
                obj_meta.rect_params.border_color.set(1.0, 0.0, 0.0, 1.0)  # Red
            else:
                # Set to blue for all other classes
                obj_meta.rect_params.border_color.set(0.0, 0.0, 1.0, 1.0)  # Blue

            # Set the font size for the labels (smaller size)
            obj_meta.text_params.font_params.font_size = 8  # Adjust this value as needed for smaller size
            ...
            confidence = round(obj_meta.confidence, 2)  # Convert to percentage
            obj_meta.text_params.display_text = f"{pgie_classes_str[obj_meta.class_id]} {confidence}"  # Label + Confidence

            # Count the relevant objects
        ...
        total_alert_objects = (obj_counter[PGIE_CLASS_ID_SCAPCOL] +
                               obj_counter[PGIE_CLASS_ID_SCAP] +
                               obj_counter[PGIE_CLASS_ID_SCOL])

        alert_buffers[source_index].append(1 if total_alert_objects > 0 else 0)

        if sum(alert_buffers[source_index]) >= ALERT_THRESHOLD:
            print(f"Alert condition met for camera {source_index}. Logging to CSV.")
            log_alert(source_index)
