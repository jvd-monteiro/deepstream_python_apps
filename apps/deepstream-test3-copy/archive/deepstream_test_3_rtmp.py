...
def pgie_src_pad_buffer_probe(pad, info, u_data):
    ...
    while l_frame is not None:
        ...
        source_index = frame_meta.pad_index  # Identify the camera/source
        frame_number=frame_meta.frame_num
        l_obj=frame_meta.obj_meta_list
        num_rects = frame_meta.num_obj_meta

        # Initialize alert buffer for the current camera index if it doesn't exist
        ...
        # Determine if an alert is present in this frame
        total_alert_objects = (obj_counter[PGIE_CLASS_ID_SCAPCOL] +
                               obj_counter[PGIE_CLASS_ID_SCAP] +
                               obj_counter[PGIE_CLASS_ID_SCOL])

        # Append to the alert buffer (1 if alert, 0 otherwise)
        alert_buffers[source_index].append(1 if total_alert_objects > 0 else 0)

        # Check if the alert condition is met (at least 100 out of the last 150 frames have alerts)
        if sum(alert_buffers[source_index]) >= ALERT_THRESHOLD:
            ...
            # Optionally clear the alert buffer after logging
